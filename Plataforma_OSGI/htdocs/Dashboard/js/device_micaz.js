...
device.prototype.setLight = function(lightObj) {
    ...
    if (lightObj != null) {
       // alert("Obj=" + lightObj + ", Got light val: " + lightObj.light);

        // The lightObj could be a single integer ...
        intVal = parseInt(lightObj.light);
        if (!isNaN(intVal)) {
...
device.prototype.setTemp = function(tempObj) {
    ...
    if (tempObj != null) {
       // alert("Obj=" + tempObj + ", Got temp val: " + tempObj.temp);

        // The lightObj could be a single integer ...
        intVal = parseInt(tempObj.temp);
        if (!isNaN(intVal)) {
...
device.prototype.setHum = function(humObj) {
	...
	alert("Valor de humedad no nulo "+humObj);
        
        // The lightObj could be a single integer ...
        intVal = parseInt(humObj.hum);
		...
		this.hum.value.push("Sensor not available");
	    this.hum.value.shift(); // remove first element
		alert("Valor de humedad "+this.hum.value[9]);
	}
}
